...
async function loadSinglePlugin(filePath, category) {
  try {
    ...
    // Get plugin metadata from file content
    const fileContent = fs.readFileSync(filePath, "utf8")
    const metadata = parsePluginMetadata(fileContent)

    // Get command name from filename if metadata doesn't specify
    const fileName = path.basename(filePath, ".js")
    const commandName = metadata.name || fileName

    // Override category if specified in metadata
    const pluginCategory = metadata.category || category
...
/**
 * Parse plugin metadata from file content
 * @param {string} content - File content
 * @returns {Object} - Plugin metadata
 */
const parsePluginMetadata = (content) => {
  const metadata = {
    name: null,
    category: null,
    description: null,
  ...
  if (usageMatch) metadata.usage = usageMatch[1].trim()

  // Check if plugin is owner-only
  const ownerMatch = content.match(/@owner\s+(true|false)/)
  if (ownerMatch) metadata.owner = ownerMatch[1].trim() === "true"
